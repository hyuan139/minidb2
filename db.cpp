					...
					else
					{
						...
						if (column_exists && column_is_int)
						{
							...
							else
							{
									...
									while (j < tab_entry->num_columns)
									{
										...
										if (strcmp(condition_columnName, column_names[j]) == 0)
										{
											if (atoi(column_type[j]) == T_INT)
											{
												...
												strcat(hexValue, temp);
												long decimal = strtol(hexValue, NULL, 16);
												if (decimal == condition_values_int)
												{
								...
								value = NULL;
								offset = 0;
								for (i = 0; i < num_matching_records; i++)
								{
									while (j < tab_entry->num_columns)
									{
										value = NULL;
										if (atoi(column_type[j]) == T_INT)
										{
											...
											memcpy((void *)((char *)value), (void *)((char *)matchingRecords + (i * old_header->record_size) + offset), sizeof(int));
											offset += sizeof(int);
											char hexValue[16];
											char temp[16];
											memset(hexValue, '\0', 16);
										...
										else if ((atoi(column_type[j]) == T_CHAR) || (atoi(column_type[j]) == T_VARCHAR))
										{
											// process as string
											value = (char *)calloc(1, atoi(column_length[j]));
											offset += 1; // account for btye for length of the value
											memcpy((void *)((char *)value), (void *)((char *)matchingRecords + (i * old_header->record_size) + offset), atoi(column_length[j]));
				...
				if (cur->tok_value == EOC)
				{
					...
					else
					{
						for (i = 0, col_entry = (cd_entry *)((char *)tab_entry + tab_entry->cd_offset); i < tab_entry->num_columns; i++, col_entry++)
						{
							...
							{
								column_exists = true;
								if (col_entry->col_type == T_CHAR || col_entry->col_type == T_VARCHAR)
								{
									column_is_string = true;
								}
							}
						}
						if (column_exists && column_is_string)
						{
							if ((fhandle = fopen(filename, "rbc")) == NULL)
							{
								rc = FILE_OPEN_ERROR;
									...
									while (j < tab_entry->num_columns)
									{
										...
										if (strcmp(condition_columnName, column_names[j]) == 0)
										{
											if ((atoi(column_type[j]) == T_CHAR) || (atoi(column_type[j]) == T_VARCHAR))
											{
												...
												offset += atoi(column_length[j]);
												if (strcmp(condition_values_string, value) == 0)
												{
													memcpy((void *)((char *)matchingRecords + recordOffset), (void *)((char *)records + (i * old_header->record_size)), old_header->record_size);
													recordOffset += old_header->record_size;
													num_matching_records += 1;
									...
									while (j < tab_entry->num_columns)
									{
										...
										if (strcmp(condition_columnName, column_names[j]) == 0)
										{
											if ((atoi(column_type[j]) == T_CHAR) || (atoi(column_type[j]) == T_VARCHAR))
											{
												...
												memcpy((void *)((char *)value), (void *)((char *)records + (i * old_header->record_size) + offset), atoi(column_length[j]));
												offset += atoi(column_length[j]);
												if (strcmp(value, condition_values_string) < 0)
												{
													memcpy((void *)((char *)matchingRecords + recordOffset), (void *)((char *)records + (i * old_header->record_size)), old_header->record_size);
													recordOffset += old_header->record_size;
													num_matching_records += 1;
...
int sem_select_project(token_list *t_list)
{
	...
	bool cond_column_exists = false;
	char condition_columnName[MAX_TOK_LEN];
	bool cond_column_is_int = false;
	bool cond_column_is_string = false;
	...
	else
	{
		...
		else
		{
			...
			else if (cur->tok_value == K_WHERE && cur->next->tok_value != EOC)
			{
				cur = cur->next;
				strcpy(condition_columnName, cur->tok_string); // copy column name
				cur = cur->next;
				...
				else
				{
					if (cur->tok_value == S_EQUAL)
					{
						...
						if (cur->tok_value == INT_LITERAL)
						{
							...
							if (cur->tok_value == EOC)
							{
								for (i = 0, col_entry = (cd_entry *)((char *)tab_entry + tab_entry->cd_offset); i < tab_entry->num_columns; i++, col_entry++)
								{
									...
									if (strcmp(condition_columnName, col_entry->col_name) == 0)
									{
										cond_column_exists = true;
										if (col_entry->col_type == T_INT)
										{
											cond_column_is_int = true;
										}
									}
								}
								projected_column_index = 0;
								...
								if (columns_exist && cond_column_exists && cond_column_is_int)
								{
									projected_column_index = 0; // reset
									int length_for_small_column = 0;
									if (length_for_print < 10)
									...
									else
									{
										...
										// print end divider
										print_separator(sum_table_length + 1);
										printf("(%d) rows found", num_matching_records);
										fclose(fhandle);
									}
								}
								else
						...
						else if (cur->tok_value == STRING_LITERAL)
						{
							...
							if (cur->tok_value == EOC)
							{
								...
								if (columns_exist && cond_column_exists && cond_column_is_string)
								{
									projected_column_index = 0; // reset
									int length_for_small_column = 0;
									if (length_for_print < 10)
					...
					else if ((cur->tok_value == K_IS) && (cur->next->tok_value == K_NULL))
					{
						...
						if (cur->tok_value == EOC)
						{
							...
							else if (columns_exist && cond_column_exists && cond_column_is_string)
							{
								projected_column_index = 0; // reset
								int length_for_small_column = 0;
								if (length_for_print < 10)
							...
							if (column_exists)
							{
								if (valid_aggregate_column)
								{
									...
									else
									{
										...
										// print value
										printf("%*d\n", sum_table_length + 5, sum);
										print_separator(sum_table_length + 5);
									}
								}
								else
								{
									rc = AGGREGATE_INVALID_COLUMN_TYPE;
									cur->tok_value = INVALID;
								}
							}
							else
							...
							else if (cur->tok_value == K_WHERE && cur->next->tok_value != EOC)
							{
								...
								if (cur->tok_value == S_EQUAL)
								{
									...
									if (cur->tok_value == INT_LITERAL)
									{
										...
										if (cur->tok_value == EOC)
										{
											...
											if (column_exists && column_is_int && valid_aggregate_column)
											{
												if ((fhandle = fopen(filename, "rbc")) == NULL)
												{
													rc = FILE_OPEN_ERROR;
									...
									else if (cur->tok_value == STRING_LITERAL)
									{
										...
										if (cur->tok_value == EOC)
										{
											...
											if (column_exists && column_is_string && valid_aggregate_column)
											{
												if ((fhandle = fopen(filename, "rbc")) == NULL)
												{
													rc = FILE_OPEN_ERROR;
								...
								else if ((cur->tok_value == K_IS) && (cur->next->tok_value == K_NULL))
								{
									...
									if (cur->tok_value == EOC)
									{
										...
										if (column_exists && column_is_int && aggregate_column_exists && valid_aggregate_column)
										{
											if ((fhandle = fopen(filename, "rbc")) == NULL)
											{
												rc = FILE_OPEN_ERROR;
										...
										else if (column_exists && column_is_string && aggregate_column_exists && valid_aggregate_column)
										{
											if ((fhandle = fopen(filename, "rbc")) == NULL)
											{
												rc = FILE_OPEN_ERROR;
							...
							if (column_exists)
							{
								if (valid_aggregate_column)
								{
									...
									else
									{
										...
										// print value
										avg = (float)(sum / old_header->num_records);
										...
										if (cur->tok_value == EOC)
										{
											...
											if (column_exists && column_is_int && valid_aggregate_column)
											{
												...
												else
												{
													...
													{
														avg = (float)(sum / num_matching_records);
													}
													printf("%*.2f\n", sum_table_length + 5, avg);
													print_separator(sum_table_length + 5);
												}
								...
								if (cur->tok_value == S_EQUAL)
								{
									...
									else if (cur->tok_value == STRING_LITERAL)
									{
										...
										if (cur->tok_value == EOC)
										{
											...
											if (column_exists && column_is_string && aggregate_column_exists)
											{
												if ((fhandle = fopen(filename, "rbc")) == NULL)
												{
													rc = FILE_OPEN_ERROR;
				...
				else
				{
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									{
										rc = COLUMN_NOT_CHAR_VARCHAR_TYPE;
										cur->tok_value = INVALID;
									}
								}
							}
					...
					else if (cur->tok_value == K_NULL)
					{
						...
						if (cur->tok_value == K_WHERE)
						{
							...
							else if (column_update_is_string)
							{
								if (cur->tok_value == S_EQUAL)
								{
									cur = cur->next;
											...
											else
											{
												...
												if (column_update_is_string && compare_column_exists && column_compare_is_int)
												{
													...
													else
													{
														new_header = (table_file_header *)calloc(1, file_stat.st_size);
														memcpy((void *)((char *)new_header), (void *)old_header, old_header->file_size);
														int len_updated_value = 0; // NULL for string is 0
														char *newColumnValueBuffer;
														for (i = 0; i < old_header->num_records; i++)
														{
															while (j < tab_entry->num_columns)
						...
						{
							// update on column to NULL no where option
							if (cur->tok_value != EOC)
							{
								rc = INVALID_UPDATE_DEFINITION;
								cur->tok_class = error;
							...
							{
								// update memcpy with necessary info if updated column is int or string type
								for (i = 0; i < tab_entry->num_columns; i++)
								{
									if (strcmp(column_to_update, column_names[i]) == 0)
									{
								...
								if (column_update_is_int)
								{
									...
									{
										printf("Set column to be NULL, and column type is type int\n");
										// Read the old header information from the tab file.
										fstat(fileno(fhandle), &file_stat);
										old_header = (table_file_header *)calloc(1, file_stat.st_size);
										fread(old_header, file_stat.st_size, 1, fhandle);
								...
								{
									printf("Set column to be NULL, and column type is type string\n");
									if ((fhandle = fopen(filename, "rbc")) == NULL)
									{
										printf("Error while opening %s file\n", filename);
										rc = FILE_OPEN_ERROR;
									...
									else
									{
										...
										new_header = (table_file_header *)calloc(1, file_stat.st_size);
										memcpy((void *)((char *)new_header), (void *)old_header, old_header->file_size);
										int len_updated_value = 0; // NULL for string value is 0
										char *reset_columnBuffer;
										for (i = 0; i < old_header->num_records; i++)
										{
											while (j < tab_entry->num_columns)
